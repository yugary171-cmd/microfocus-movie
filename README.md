...
未配置 `VITE_API_BASE_URL` 时，后台是 Mock 演示模式：输入符合格式的邮箱、8 位以上密码和 6 位验证码即可进入，角色下拉框只用于体验权限边界，不代表真实账号鉴权。配置 API 地址后才进入真实模式；真实管理员登录由 API 校验密码、JWT 和 TOTP，一次登录会话过期后需要重新登录。系统不提供开放注册，管理员账号由部署/运维通过 seed 或受控管理流程创建。后台各角色怎么上剧、审核、发布和应急操作，见 [后台管理系统操作说明](docs/admin-handbook.md)。
