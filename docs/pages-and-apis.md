...
| 我的 | `apps/uniapp/src/pages/my/index.vue` | 显式登录（先拉起微信头像昵称授权）、个人信息编辑入口、观看历史和继续观看 | `POST /v1/auth/wechat`、`GET /v1/me/history`、`GET/PATCH /v1/me/profile` |
...
| `POST /v1/playback/leases/:leaseId/recover` | 用户 JWT + 近期重新认证证明；按认证用户限频 | `deviceId` 最长 128（`DEVICE_ID_MAX_LENGTH`）；`reason` 最长 191（`ENTITY_ID_MAX_LENGTH`）；`wechatCode` 限长；恢复事件写入按上述上限截断 | 核验媒体交付证据后幂等结算、释放或转客服；无真实 VOD 交付日志时 UNCONFIRMED 只释放不扣费；自动宽限受滚动风险上限约束 |
...
| `POST /v1/admin/callback-events/:eventId/replay` | ADMIN + `Idempotency-Key`（trim、最长 128）；空白或超长在 handler 拒绝，写 Guard 仍先占桶 | 死信事件 ID、原因和审批记录（写入按 `ADMIN_REASON_MAX_LENGTH=300` 截断） | 将 RETRYABLE_FAILURE/DEAD_LETTER 事件受审计地迁回 PROCESSING，沿用原 provider 事件幂等键；保留期内若有加密规范化载荷则立即执行；超过 30 天密文会被清除，重放只解锁 |
