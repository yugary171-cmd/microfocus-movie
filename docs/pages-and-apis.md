...
不得仅为模仿参考产品新增页面。若未来将剧场定义为正式推荐流，必须复用同一套租约、权益和心跳契约，不能使用 Demo URL 旁路播放；该变化需要独立产品决策。
...
除登录接口外，所有管理接口均需管理员 JWT；表中角色是服务端必须执行的最小权限。

| 方法与路径 | 角色 | 所有权/状态约束 | 用途 |
| --- | --- | --- | --- |
| `POST /v1/admin/auth/login` | 公开 | 邮箱、密码、OTP | 换管理员 JWT |
| `GET /v1/admin/dashboard` | 全部管理员角色 | 无写权限 | 状态计数与闸门摘要 |
...
| `POST .../:id/publish`、`POST .../:id/offline` | ADMIN | 必须满足状态、权利、媒体和发布闸门 | 发布与下架 |
| `GET /v1/admin/audit-logs` | ADMIN | 只读、不可篡改 | 审计查询 |
...
| `POST /v1/admin/callback-events/:eventId/replay` | ADMIN + `Idempotency-Key` | 死信事件 ID、原因和审批记录 | 将 RETRYABLE_FAILURE/DEAD_LETTER 事件受审计地迁回 PROCESSING，沿用原 provider 事件幂等键 |
