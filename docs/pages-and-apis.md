...
| `GET /v1/catalog` | 公开、按连接 IP 限频 | 无 | `featured/latest/popular/categories`；只含已发布且权利有效内容 |
...
除登录接口外，所有管理接口均需管理员 JWT；表中角色是服务端必须执行的最小权限。写操作（POST/PATCH/PUT/DELETE）按认证管理员身份限频，不使用可伪造的客户端字段。
