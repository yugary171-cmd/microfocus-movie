...
- 2026-08-20：剧集抽屉打开时补偿滚动条占位，避免页面左右跳动；表格行高收紧；时长改为同一行「秒数 + 分:秒」；集标题选填，空标题在观看端显示「第 N 集」。
