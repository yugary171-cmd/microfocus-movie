...
- 2026-08-15 当前工作区执行 `npm run check` 通过；该结果覆盖 typecheck、单元/组件测试和构建，不等于 HTTP E2E、真实 MySQL 并发、真机或真实 provider 验收。HEAD `0efab06`。
...
4. 用样片中位时长把 600 秒写成「约 N 集 + 过期时间 + 仅本剧」，固定 5 人无提示任务测试。
