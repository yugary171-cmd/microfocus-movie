...
- API、管理后台、uni-app 和原生过渡小程序已形成首轮内部 Mock 实现；当前工作区仍有大量未提交和未跟踪变更，不能视为可引用发布快照。
...
| 内容管理 | 剧目/剧集、权利版本、媒体版本、审核、发布/下架和基础审计 | EDITOR 列表、详情和修改尚未完整执行所有权过滤；ADMIN 当前仍可编辑/审核，REVIEWER 可查看审计，目标角色隔离未完成；真实 VOD 发布链路未实现 |
| 奖励与权益 | challenge、基础回调占用、grant、FEFO debit、24 小时过期 | 可信广告验证未接真实平台；人工补偿不幂等；adjustment、晚到奖励和死信重放尚未实现 |
...
1. 收紧管理端角色边界：EDITOR 仅访问/修改本人内容，ADMIN 不兼任编辑或审核，审计日志仅 ADMIN 可见，并补齐越权测试。
2. 为人工补偿增加 `Idempotency-Key`、数据库唯一约束、契约类型和重放测试。
3. 建立管理端和观看端统一的 Live API 构建注入；外部构建缺少 API 地址或包含 Demo 媒体时必须失败。
4. 按实现矩阵逐项补齐匿名会话、播放 reservation/恢复、权益 adjustment、注销和死信重放；每项先更新 `packages/contracts`，再实现服务端和客户端。
5. 当前未提交工作区完成范围审查、敏感信息扫描和重新验证后，再由人工决定是否创建 Git 快照；不自动提交或推送。
