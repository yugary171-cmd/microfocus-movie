...
| 客户端 | 管理端和两套观看端的 Mock 主路径、uni-app 平台适配层；Live API URL 注入与外部构建 Demo 媒体扫描；观看端匿名 viewer 会话；登录后播放先查活动租约并可宽限恢复 | 目标注销交互尚未实现 |
...
1. 按实现矩阵逐项补齐权益 adjustment、注销和死信重放；每项先更新 `packages/contracts`，再实现服务端和客户端。
2. 不自动推送；后续提交需人工确认。
