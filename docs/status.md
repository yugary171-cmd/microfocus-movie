...
4. 播放器与额度文案已按样片中位时长把 `REWARD_SECONDS` 写成「约 N 集 + 过期时间 + 仅本剧 + 广告未看完不发奖」。广告未看完/无填充/加载失败/验证中/确认失败文案已区分。固定 5 人无提示任务测试仍待人工。
