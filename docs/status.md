...
- 2026-08-14：`POST /v1/admin/uploads/sign` 在签发成功后写入 `UPLOAD_SIGNED` 审计，目标为剧集，元数据只含 `dramaId`，不记录签名 URL。已实现的权利版本、媒体资产、媒体审核、注销申请列表和熔断 provider 路径收入 `API_ROUTES.admin`；管理端改走该契约。不把灰度指标写成已接入。
