...
| 客户端 | 管理端和两套观看端的 Mock 主路径、uni-app 平台适配层；Live API URL 注入与外部构建 Demo 媒体扫描；观看端匿名 viewer 会话；登录后播放先查活动租约并可宽限恢复；「我的」可申请注销并查询进度；ADMIN 可在客服核验后补发注销查询令牌；目录、剧目详情和搜索按连接 IP 限频；搜索最多 100 页；观看历史按认证用户限频；注销进度查询按连接 IP 限频 | 完整法定清理尚未实现 |
