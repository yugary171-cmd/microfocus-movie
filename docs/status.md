...
- 2026-08-15：「我的」登录先拉起微信头像昵称授权；取消保持游客。退出登录确认后调用 `clearStoredSession()`，回到游客态；与申请注销分开。
...
- 2026-08-17：观看端未登录点击锁定集时，双端详情页直接在当前点击流程拉起微信头像昵称授权与登录 code 流程，不新增独立登录页；授权后刷新本剧额度，已登录但额度不足时进入激励广告解锁流程。免费集保持匿名可看。
