...
import { cloneLocalComments } from "../components/comment-sheet/comments";
...
function mockAuthor(name: string): CommentView["author"] {
  return {
    id: `mock-author-${name}`,
    displayName: name,
    avatarUrl: null,
...
function ensureDramaComments(dramaId: string): MockComment[] {
  ...
  for (const sample of cloneLocalComments()) {
    seeded.push({
      id: `${dramaId}-${sample.id}`,
      author: mockAuthor(sample.author),
      targetType: "DRAMA",
      dramaId,
      targetUserId: null,
      episodeId: null,
      ...
      likedByMe: false,
      replyCount: sample.replies.length,
      status: COMMENT_STATUSES[0],
      createdAt: `${sample.createdAt}T00:00:00.000Z`
    });
    for (const reply of sample.replies) {
      seeded.push({
        id: `${dramaId}-${reply.id}`,
        author: mockAuthor(reply.author),
        targetType: "DRAMA",
        dramaId,
        targetUserId: null,
        episodeId: null,
        parentCommentId: `${dramaId}-${sample.id}`,
        replyToUserId: `mock-author-${sample.author}`,
        body: reply.text,
        likeCount: reply.likeCount,
        likedByMe: false,
        replyCount: 0,
        status: "VISIBLE",
        createdAt: `${reply.createdAt}T00:00:00.000Z`
      });
    }
  }
