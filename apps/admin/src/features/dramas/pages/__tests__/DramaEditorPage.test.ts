...
import DramaEditorPage from "@/features/dramas/pages/DramaEditorPage.vue";
import { createDrama } from "@tests/fixtures";
...
describe("DramaEditorPage", () => {
  ...
  it("uses contract field limits and blocks save until type and tags are chosen", async () => {
    const wrapper = mount(DramaEditorPage);
    await flushPromises();
  ...
  it("lets a system administrator fill and save a new drama", async () => {
    ...
    const wrapper = mount(DramaEditorPage, { attachTo: document.body });
    await flushPromises();
