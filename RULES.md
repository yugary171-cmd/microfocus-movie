...
- 管理端视觉尺度全局生效，后续页面与其它端的同类控件沿用同一套，不另发明：单行输入/选择 32px 高；按钮 32px 高、字号 12px、字重 500；间距只允许 4 / 8 / 16 / 24px（`--space-1` 到 `--space-4`）；标签在上、控件在下时间距固定 8px（`.field` 的 `gap`）。1px 描边、控件高度、圆角、图标盒不属于间距阶梯。
